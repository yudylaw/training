...
/*			$.get( U( "widget/UserCount/getUnreadCount" ), function( msg ) {	
			...
			$.get(U('widget/UserCount/getUnreadCount'), function(msg) {	
				if ('undefined' == typeof msg.data || msg.status != 1) {
					...
					var unread_total = unread_notify+unread_atme+unread_digg+unread_comment+unread_message;
					...
					unread_total > 0 && $unread_total.append('<a class="num" rel="notice-btn" href="javascript:void(0);" num="' + unread_total + '">' + (unread_total>9?'···':unread_total) + '</a>');
		...
		try{
			...
			setInterval(function(){
				...
				if(num && parseInt(num[1]) > 0){
					$unread_total.append('<a class="num" rel="notice-btn" href="javascript:void(0);" num="' + num[1] + '">' + (num[1]>9?'···':num[1]) + '</a>');
