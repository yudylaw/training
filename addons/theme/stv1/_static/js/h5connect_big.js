...
qcVideo('Base', function(util) {
	...
	Base.prototype.fire = function(event, opt) {
		...
		if (cache && cache[event]) {
			util.each(cache[event], function(fn) {
				fn.call(global, opt);
			});
		}
	};
		...
		constructor: function(link, im, uniqueImVal, done, fail) {
			...
			var roll = function() {
					...
					if (status == 'uninit') {
						...
						if (status == 'fail') {
							touristTlsLogin(self.im['sdkAppID'], self.im['accountType'], function(info, error) {
								if (!error) {
...
qcVideo('constants', function() {
	return {
		...
		TDBANK_REPORT_API: "http://tudg.qq.com/dataimport/ImportService",
		OK_CODE: '0',
		ERROR_CODE: {
			TIME_OUT: '10000',
			REQUIRE_PWD: '11046',
...
qcVideo('FullScreenApi', function(version) {
	...
	if (browserApi && !version.IS_MOBILE) {
		fullscreenAPI.supportFullScreen = true;
		for (i = 0; i < browserApi.length; i++) {
			fullscreenAPI[specApi[i]] = browserApi[i];
		}
	} else {
		fullscreenAPI.supportFullScreen = false;
	}
	return fullscreenAPI;
});;
qcVideo('H5', function(constants, api, util, Base, config, startup_tpl, TDBankReporter) {
	var $;
	return Base.extend({
		verifyDone: function(data) {
			...
			h5player['render'](me.store);
			me.sdklink.setSwf(h5player.mediaPlayer);
			delete me.sdklink;
		},
		...
		askDoor: function(firstTime, pass) {
			var me = this,
				store = me.store,
				key, address = constants.SERVER_API + '?interface=Vod_Api_GetPlayInfo&1=1';
			...
			me.loading(true);
			api.request(address, function(ret) {
				me.loading();
				var code = ret['retcode'] + '',
					data = ret['data'];
				TDBankReporter.pushEvent('connectPlayCgiH5', {
					setting: {
				...
				if (code == constants.OK_CODE) {
					...
					if (isPwdError && firstTime) {
						me.renderPWDPanel();
					} else {
						var errorMsg = constants.ERROR_MSG[code] || '';
						if (!errorMsg || !isPwdError) {
							errorMsg += ' Code:(' + code + ')';
						}
						me.erTip(errorMsg, isPwdError);
					}
				}
			});
		},
		...
		constructor: function(_$, targetId, opt, eid, link) {
			...
			me.$error = $out.find('[data-area="error"]');
			me.$loading = $out.find('[data-area="loading"]');
			...
			if ( !! third && !! third['urls'] && !! third['duration']) {
				var key, data = {
					file_info: {
...
qcVideo('Player', function(util, Base, version, css, H5, Swf, SwfJsLink, constants, TDBankReporter, BJ_REPORT, config) {
	...
	function matchIfmWidth(opt, ele) {
		...
		var tempW = css.getVisibleWidth();
		var tempH = css.getVisibleHeight();
		if (opt.width > tempW || opt.height > tempH) {
			if (opt.width > tempW) {
				tempH = tempW / rate;
			}
			if (opt.height > tempH) {
			...
			ele.style.width = (opt.width = tempW) + 'px';
			ele.style.height = (opt.height = tempH) + 'px';
			return false;
		} else {
	...
	function setSuitableWH(opt, ele) {
		var width = opt.width,
			...
			rate = width / height,
			minPix = 4;
		if (pW < minPix && ele.parentNode) {
			var pEle = ele.parentNode;
			while (true) {
				if (!pEle || pEle === document.body) {
					pW = css.getVisibleWidth();
					pH = css.getVisibleHeight();
					break;
				} else {
					pW = css.getWidth(pEle);
					pH = css.getHeight(pEle);
					if (pW > minPix) {
						break;
					}
				}
		...
		if (pH < minPix && pW > minPix) {
			pH = pW / rate;
		}
		if (opt['match_page_width']) {
			if (!matchIfmWidth(opt, ele)) {
				return;
			}
		}
		if (pW < minPix) {
	...
	return Base.extend({
		...
		verifyDone: function(targetId, opt, listener) {
			...
			var inWhiteAppId = false;
			if ( !! opt && !! opt['app_id']) {
				inWhiteAppId = !version.IS_MOBILE;
				opt.inWhiteHlsJs = inWhiteAppId;
			}
			var ver = version.getVodPriority(inWhiteAppId),
				eid = getEid();
			var flashAble = version.getFlashAble();
			var link = new SwfJsLink(eid, listener);
			if (ver == 'h5') {
				qcVideo.use('$', function($) {
					if (version.REQUIRE_HLS_JS) {
						qcVideo.use('Hls', function(mod) {
				...
				if (flashAble == 'lowVersion') {
					ele.innerHTML = constants.UNICODE_WORD.TIP_UPDATE_FLASH + '<a target="_blank" style="color:blue;" href="http://www.macromedia.com/go/getflashplayer">' + constants.UNICODE_WORD.TIP_CLICK_UPDATE_FLASH + '</a>'
				} else {
					new Swf(targetId, eid, opt, link);
					TDBankReporter.pushEvent('connectInitFlash', {
						setting: {
							app_id: opt.app_id
						}
					});
		...
		constructor: function(targetId, opt, listener) {
			TDBankReporter.pushEvent('connectInit', {
				setting: {
					app_id: opt.app_id
				}
			});
			...
			ableReportJsError = ableReportJsError === 0 && (opt && (opt['app_id'] in constants.BAD_JS_REPORT_WHITE_APP_IDS)) ? 1 : -1;
			if (ableReportJsError === 1) {
				BJ_REPORT.init({
					id: 1067,
					uin: 123,
					combo: 0,
			...
			if (!targetId || !document.getElementById(targetId)) {
				...
				if (video) {
					if (!video['duration']) {
						...
						return this.verifyDone(targetId, opt, listener);
					}
				}
...
qcVideo('Swf', function(Base, config, JSON, LinkIm) {
	var getHtmlCode = function(option, eid) {
			...
			var flashvars = 'auto_play=' + option.auto_play + '&version=1&refer=' + option.refer + '&jscbid=' + eid;
			var VMode = option.VMode || option.WMode || 'window';
			flashvars += !! option.disable_full_screen ? '&disable_full_screen=1' : '&disable_full_screen=0';
			flashvars += !! option.debug ? '&debug=1' : '';
			if (option.file_id) {
				flashvars += '&file_id=' + option.file_id;
			}
			if (option.app_id) {
				flashvars += '&app_id=' + option.app_id;
			}
			...
			if (option.videos && option.videos.length) {
				flashvars += '&videos=' + encodeURIComponent(JSON.stringify(option.videos));
			}
			if ( !! option.third_video) {
				flashvars += '&third_video=' + encodeURIComponent(JSON.stringify(option.third_video));
			}
			if (option.skin) {
				flashvars += '&skin=' + encodeURIComponent(JSON.stringify(option.skin));
			}
			if (option.stop_time) {
				flashvars += '&stop_time=' + option.stop_time;
			}
			if (option.remember) {
				flashvars += '&remember=' + option.remember;
			}
			if (option.capture) {
				flashvars += '&capture=' + option.capture;
			}
			if (option.stretch_patch) {
				flashvars += '&stretch_patch=' + option.stretch_patch;
			}
...
qcVideo('SwfJsLink', function(util, JSON, H5) {
	var global = window;
	...
	var SwfJsLink = function(id, listeners) {
			...
			if (!listeners || type == 'function') {
				me.listeners['playStatus'] = listeners ||
				function() {};
			} else if (type == 'object') {
	...
	util.each(['volume', 'duration', 'currentTime', 'clarity', 'allClaritys'], function(name) {
	...
	util.merge(SwfJsLink.prototype, {
		setSwf: function(obj) {
			this.swf = obj;
			this.tecGet = 'sdk_tecGet';
			this.operate = 'sdk_operate';
			this.source = 'sdk_source';
			this.barrage = 'sdk_barrage';
			this.close_barrage = 'sdk_close_barrage';
		},
		isJsPlayer: function() {
			return this.tecGet == 'sdk_tecGet';
		},
		getSwf: function() {
			var me = this;
			if (!me.swf) {
		...
		resize: function(w, h) {
			...
			if (swf) {
				if (this.isJsPlayer()) {
					this.getSwf()[this.operate]('resize', w, h);
				} else {
		...
		changeVideo: tryIt(function(opt) {
			if (this.isJsPlayer()) {
				...
				var link = this;
				opt.width = opt.width || args.width;
				opt.height = opt.height || args.height;
				opt.refer = args.refer;
				qcVideo.use('$', function(mod) {
					new H5(mod, args['targetId'], opt, args['eid'], link);
				});
			} else {
				return this.getSwf()[this.source](opt['file_id'], opt['app_id'] || '', !! opt.videos && opt.videos.length ? JSON.stringify(opt.videos) : '', !! opt.third_video ? JSON.stringify(opt.third_video) : '', opt['auto_play'] ? 1 : 0);
			}
		}),
...
qcVideo('TDBankReporter', function(MD5, constants) {
	...
	var _storeInfo = {};
	var _playId = '';
	var _ifFirstPlaying = true;
	var _vid = '';
	var _reportStep = 2;
	var pushPlayEvent = function(event, storeInfo) {
			_storeInfo = storeInfo || {};
			switch (event) {
			case "playing":
				if (_ifFirstPlaying || _vid != storeInfo.vid) {
					_vid = storeInfo.vid;
					_playId = MD5.md5(new Date().getTime());
					_reportStep = 2;
					_detectOS();
					sendReq();
				}
				_ifFirstPlaying = false;
				break;
			case "seeking":
				break;
			case "playEnd":
				_ifFirstPlaying = true;
				break;
			case "suspended":
				break;
			case "connectInit":
	...
	var _detectOS = function() {
			var detect = function() {
					var sUserAgent = navigator.userAgent;
					if (/Android/i.test(sUserAgent)) return "Android";
					if (/iPhone|iPad|iPod/i.test(sUserAgent)) return "iOS";
					if (/Windows/i.test(sUserAgent)) return "Windows";
					if (/Mac/i.test(sUserAgent)) return "Mac";
					return "other";
				}
			switch (detect()) {
			case "Android":
	...
	var sendReq = function() {
			...
			var url = constants.TDBANK_REPORT_API + '?m=dataImport&p=["100043","' + reqData + '"]';
			try {
...
qcVideo('h5player', function($, Base, constants, util, MediaPlayer, version) {
	...
	return Base.instance({
		...
		render: function(opt) {
			...
			if (!version.ABLE_H5_HLS && !version.ABLE_H5_APPLE_HLS && (!version.REQUIRE_HLS_JS || !parameter['inWhiteHlsJs'])) {
				constants.DEFINITION_PRIORITY = constants.JUST_MP4_DEFINITION_PRIORITY
			}
			var resolution = player_info['resolution_type'];
			if (resolution !== undefined) {
				store.setting.resolution = resolution;
			}
			store.imgUrls = fileInfo.image_video.imgUrls;
			store.image_url = fileInfo.image_url;
			store.duration = fileInfo.duration | 0;
			store.logo['url'] = player_info['logo_pic'];
			store.logo['location'] = player_info['logo_location'];
			store.logo['redirect'] = player_info['logo_url'];
			store.customization = getSpecArgs(opt);
			store.keepArgs = opt.keepArgs;
			store.sdk_method = opt.sdk_method;
			store.vid = fileInfo.vid;
			...
			var hasMoreFormat = false,
				sourceIsMp4 = false;
			$.each(video, function(_, item) {
				var def = item['definition'] | 0;
				if (def == 0) {
					if (item['url'].toLowerCase().indexOf('.mp4') > 0) {
						sourceIsMp4 = true;
					}
				} else if (def > 1) {
					hasMoreFormat = true;
			...
			if (sourceIsMp4 && !hasMoreFormat) {
				constants.DEFINITION_PRIORITY = constants.ONLY_MP4_NO_TRANS;
				opt['disable_drag'] = true;
				store.customization = getSpecArgs(opt);
			}
...
qcVideo('Bottom_container', function($, UICom, Drag, util, UiControl, constants, FullScreenApi, version) {
	...
	var FIRE = constants.FIRE;
	var WORD = constants.UNICODE_WORD;
	var svg_info = {
		play: {
			title: WORD.PLAY,
	...
	return UICom.extend({
		...
		init: function() {
			var me = this;
			me.subs = {};
			me.$el.find('[sub-component]').each(function() {
				var $me = $(this);
			...
			if (me.subs['volume_slider']) {
				me.volumeDrag = (new Drag(me.subs['volume_slider'], me.subs['volume_bg'])).on(Drag.EVENT_DRAG_STOP, function() {
					me.fire(FIRE, {
				...
				if (keyCode == 38) {
					if (me.volumeDrag) {
						me.fire(FIRE, {
							event: EVENT.UI_SET_VOLUME,
							value: {
								volume: Math.min(1, me.volumeDrag.getPercent() + 0.1)
							}
						});
					}
				} else if (keyCode == 40) {
					if (me.volumeDrag) {
						me.fire(FIRE, {
							event: EVENT.UI_SET_VOLUME,
							value: {
								volume: Math.max(0, me.volumeDrag.getPercent() - 0.1)
							}
						});
					}
				} else if (keyCode == 39) {
			...
			if (!FullScreenApi.supportFullScreen) {
				if (this.subs['fullscreen_btn']) {
					this.subs['fullscreen_btn'].width(0).height(0).hide().empty();
				}
				this.subs['setting'].css('right', 5);
			}
		},
		...
		enableDrag: function(bool) {
			...
			this.subs['progress_load'].css('cursor', cursor);
			this.subs['progress_bg'].css('cursor', cursor);
		},
		catchControlStatusChange: function(s, obj) {
			...
			case (m.QUIT_FULL):
				...
				break;
			case (m.RESIZE):
				if (obj.offset.width > 0) {
					if (obj.offset.width < 270) {
						this.subs['time_duration'].hide();
					} else {
						this.subs['time_duration'].show();
					}
				}
				break;
			case (m.END):
		...
		_setSvg: function($dom, name) {
			if ($dom.attr('now') !== name) {
				...
				$dom.attr({
					...
					if (id == item.method) {
						$(this).width('100%').height('100%').show();
					} else {
						$(this).hide();
					}
				});
		...
		_setProgress: function(obj) {
			...
			if (subs) {
				if (obj.lPercent !== undefined) {
					...
					if (me._lPercent !== lPercent) {
						me._lPercent = lPercent;
				...
				if (obj.pPercent !== undefined && me.processDrag && !me.processDrag.isMoving()) {
					...
					if (me._pPercent !== pPercent) {
						me._pPercent = pPercent;
						subs['progress_play'].css('width', (pPercent * 100) + '%');
						subs['progress_pointer'].css('left', pPercent * subs['progress_bg'].width());
					}
				}
			}
		},
		setTime: function(obj) {
			var subs = this.subs;
			if (obj.currentTime !== undefined && this.duration) {
				subs['time_played'].html(util.convertTime(obj.currentTime));
				this._setProgress({
			...
			if (obj.duration !== undefined) {
				...
				subs['time_duration'].html(' / ' + util.convertTime(obj.duration));
			}
			if (obj.loaded !== undefined && this.duration) {
				this._setProgress({
		...
		setVolume: function(vPercent) {
			...
			if (subs) {
				if (vPercent == 100) {
					me._setSvg(subs['volume_mute'], 'full_volume');
				} else if (vPercent == 0) {
					me._setSvg(subs['volume_mute'], 'mute_volume');
				} else {
					me._setSvg(subs['volume_mute'], 'volume');
				}
				vPercent = ((vPercent || 0) / 100) * volume_width;
				subs['volume_track'].width(vPercent);
				if (me.volumeDrag && !me.volumeDrag.isMoving()) {
					subs['volume_slider'].css('left', vPercent)
				}
			}
		},
		...
		on_click_play_btn: function($dom) {
			this.fire(FIRE, {
				event: EVENT[$dom.attr('now') == 'play' ? 'UI_PLAY' : 'UI_PAUSE'],
				value: {}
			});
		},
		...
		on_click_fullscreen_btn: function($dom) {
			this.fire(FIRE, {
				event: $dom.attr('now') == 'fullscreen' ? EVENT.UI_FULL_SCREEN : EVENT.UI_QUIT_FULL_SCREEN,
				value: {}
			});
		},
		_click_progress_handler: function(event) {
			if (this.processDrag.isAble()) {
				...
				var percent = ((event.pageX - $bg.offset().left) / $bg.width()).toFixed(3) - 0;
				this.debug('点击播放进度百分比==' + percent);
				this.fire(FIRE, {
					event: EVENT.UI_DRAG_PLAY,
					value: {
						percent: percent
...
qcVideo('Center_container', function($, UICom, constants, UiControl, version) {
	return UICom.extend({
		...
		init: function() {
			...
			me._fixButtonSize();
			me.subs['play'].on('mouseenter', function() {
				me.log('mouseenter', me.subs['play'].find('circle[data-opacity]'));
				me.subs['play'].find('circle[data-opacity]').attr('fill-opacity', '0.8');
			}).on('mouseleave', function() {
				me.log('mouseleave', me.subs['play'].find('circle[data-opacity]'));
				me.subs['play'].find('circle[data-opacity]').attr('fill-opacity', '0.5');
			});
		},
		...
		_fixButtonSize: function() {
			...
			var max = version.IS_MOBILE ? 2000 : 75;
			var size = Math.min(Math.min(offset.width, offset.height) * 0.3, max) / 2;
			me.subs['play'].width(size * 2).height(size * 2).css({
	...
	return UICom.extend({
		...
		_setLogo: function(obj) {
			...
			if (obj && obj.url) {
				...
				var style = "position:absolute;z-index:20;";
				var loc = obj['location'] = obj['location'] | 0;
				if (loc == 0) {
...
qcVideo('Patch', function($, UICom, UiControl, util) {
	return UICom.extend({
		...
		catchControlStatusChange: function(s, data) {
			...
			switch (s) {
			case (m.WAIT):
				me.store.happenToSdk('playStatus:seeking');
				me._poster = null;
				me.$el.hide();
				break;
			case (m.PLAY):
				me._poster = null;
				me._show_start_patch = true;
				me.$el.html('');
				me.store.happenToSdk('playStatus:playing');
				break;
			case (m.PAUSE):
				if (!me._show_start_patch) {
					me._show_start_patch = true;
					me._setPoster(me.store.getStartPatch());
					me.store.happenToSdk('playStatus:ready');
				} else {
					me._setPoster(me.store.getPausePatch());
					me.store.happenToSdk('playStatus:' + me.status.getSDKStatus());
				}
				break;
			case (m.ERROR):
				me._poster = null;
				...
				break;
			case (m.END):
				me._setPoster(me.store.getSopPatch());
				me.store.happenToSdk('playStatus:playEnd');
				break;
			case (m.RESIZE):
				if (me._poster) {
		...
		_setPoster: function(obj) {
			if (obj && obj.url) {
				var me = this,
					$el = me.$el;
				var hasRedirect = obj.redirect && obj.redirect.length > 7;
				var link = hasRedirect ? obj.redirect : 'javascript:void(0);',
					style = !hasRedirect ? 'cursor: default;' : '';
				me._poster = obj;
				var tpl = qcVideo.get('MediaPlayer_tpl');
				if (me.store.isCustomization('stretch_patch')) {
...
qcVideo('Setting', function($, UICom, constants) {
	return UICom.extend({
		...
		init: function() {
			...
			var data = [];
			var all = this.store.getUniqueNoDefinition();
			for (var key in all) {
				data.push({
		...
		on_click_definition: function($dom) {
			if (!$dom.hasClass('setting-definition-value-curr')) {
				this.hide();
				this.fire(constants.FIRE, {
					event: constants.EVENT.UI_CHOSE_DEFINITION,
					value: {
						value: $dom.attr('data-def') | 0
					}
				});
			}
		},
...
qcVideo('Drag', function($) {
	var Drag = function($el, $papa) {
			...
			me.maxWidth = 0;
			me._able = true;
			me[Drag.EVENT_DRAG_START] = me[Drag.EVENT_DRAG_STOP] = me[Drag.EVENT_DRAG_ING] = function() {};
			me.$el.on('dragstart', function(e) {
				if (me._able) {
					me._dragStartHandler.call(me, e);
				}
			}).on('drag', function(e) {
				if (me._able) {
					me._dragDragHandler.call(me, e);
				}
			}).on('dragend', function(e) {
				if (me._able) {
					me._dragEndHandler.call(me, e);
				}
	...
	$.extend(Drag.prototype, {
		_modifyPath: function(e) {
			...
			if (diff >= 0 && diff <= this._maxWidth) {
				this.$el.css('left', diff);
			}
		},
		_dragStartHandler: function(e) {
			this.__moving = true;
			this._maxWidth = this.maxWidth || this.$papa.width();
			this._startX = e.x;
			this._sourceLeft = this.$el.position().left;
			this[Drag.EVENT_DRAG_START]();
		},
		_dragDragHandler: function(e) {
			this._modifyPath(e);
			this[Drag.EVENT_DRAG_ING]();
		},
		_dragEndHandler: function(e) {
			this._modifyPath(e);
			this[Drag.EVENT_DRAG_STOP]();
			this.__moving = false;
		},
		getPercent: function() {
	...
	Drag.EVENT_DRAG_START = 'EVENT_DRAG_START';
	Drag.EVENT_DRAG_STOP = 'EVENT_DRAG_STOP';
	Drag.EVENT_DRAG_ING = 'EVENT_DRAG_ING';
	return Drag;
});;
qcVideo('MediaPlayer', function($, Base, UiControl, PlayerSystem, PlayerStore, MediaPlayer_tpl, Error, version, constants, FullScreenApi) {
	...
	return Base.extend({
		...
		__filter_play: function() {
			var me = this;
			if (!me.system.isMetaDataRendered()) {
				me.control.setWait();
				me.system.status.setSDKStatus('seeking');
			}
		},
		...
		toPlay: function(second) {
			...
			this.whenReadyToPlay = true;
			this.__filter_play();
			if (status.getRunningStatus() === 'error') {
				status.clear();
				status.setRememberKey(url);
				system.setUrl(url);
			}
			system.play(second !== undefined ? second : status.get_played());
		},
		tapEvent: 'click',
		toPxNum: function(n) {
			return (n || '').toLowerCase().replace('px', '') | 0;
		},
		ableToPlay: function(t) {
			var me = this;
			var stop_time = me.store.getCustomization('stop_time');
			if (stop_time) {
				if (me.system.status.get_played() >= stop_time) {
					return false;
				}
			}
			return true;
		...
		constructor: function(info) {
			...
			if (!setting.width || !setting.height) {
				var $p = setting.$renderTo.parent(),
					pw = $p.width(),
					ph = $p.height();
				setting.width = pw - me.toPxNum($p.css('padding-left')) - me.toPxNum($p.css('padding-right'));
				setting.height = ph - me.toPxNum($p.css('padding-top')) - me.toPxNum($p.css('padding-bottom'));
			}
			...
			var video = me.video_data = store.getVideoInfo(setting.resolution, setting.definition);
			var ableRemember = store.isCustomization('remember');
			status.setRememberKey(video.url);
			status.enableRemember(ableRemember);
			status.set_duration(store.getDuration());
			if (me.system.fileType === constants.HLS) {
				ableRemember = false;
				store.setCustomization('disable_drag', true);
			}
			store.store.mainId = mid;
			me.control = new UiControl(store, status, $container);
			me.control.enableDrag(!store.isCustomization('disable_drag'));
			me.control.enableFull(!store.isCustomization('disable_full_screen'));
			me.system.setUrl(video['url']);
			me.system.status.setRunningStatus('wait');
			status.setSDKStatus('suspended');
			me.batchOn(status, [{
				...
				fn: function(e) {
					status.setRunningStatus('error');
					me.control.setError(Error.NET_ERROR);
				}
			}, {
			...
			var playEnd = function() {
					me.playEndYet = true;
					me.whenReadyToPlay = false;
					me.control.setEnd();
					status.setRunningStatus('end');
					status.setSDKStatus('playEnd');
				};
			if (FullScreenApi.supportFullScreen) {
				var fullChange = function() {
						...
						try {
							me.control.setFull(isFull);
							me.control.setResize();
						} catch (xe) {}
					};
				$(document).on(FullScreenApi['fullscreenchange'], fullChange);
				$('#' + store.store.mainId).on(FullScreenApi['fullscreenchange'], fullChange);
			}
			if (!version.IS_MOBILE) {
				$container.on(me.tapEvent, function(e) {
					if (status.getRunningStatus() == 'play') {
						system.pause();
					}
				});
			}
			me.batchOn(me.control, [{
				...
				fn: function(e) {
					me.system.setFullScreen(true, mid);
				}
			}, {
				event: EVENT.UI_QUIT_FULL_SCREEN,
				fn: function(e) {
					me.system.setFullScreen(false, mid);
				}
			}, {
				event: EVENT.UI_PLUS_PLAY,
				fn: function(e) {
			...
			var updateShowMode = function(end) {
					if (version.IS_MOBILE) {
						if (end) {
							system.setShowMode(false);
							me.control.show();
						} else {
							system.setShowMode(true);
							me.control.hide();
						}
					}
				};
			me.batchOn(system, [{
				...
				fn: function() {
					if (me.whenReadyToPlay) {
						if (me.switchPlayTime) {
							setTimeout(function() {
								me.dragPlay(me.switchPlayTime);
								me.switchPlayTime = 0;
							}, 10);
						} else {
							me.control.setPlay();
				...
				fn: function(e) {
					...
					var a = store.getDuration();
					if (t > 1 && a && Math.abs(a - t) < 2) {
						return;
					}
					if (me.ableToPlay(t)) {
						status.setSDKStatus('suspended');
					} else {
						status.setSDKStatus('stop');
					}
					me.control.setPause();
					updateShowMode(true);
				}
			...
			system.setPlayRate(setting.playbackRate);
			me.whenReadyToPlay = false;
			if (setting.isAutoPlay && !version.IS_MOBILE) {
				setTimeout(function() {
					me.whenReadyToPlay = true;
					me.control.setWait();
					system.play();
		...
		rememberPlay: function(status, system, duration) {
			...
			var lVolume = status.get_local_volume() - 0;
			if (lVolume > 0) {
				system.volume(lVolume);
			}
			if (lPlayed > 0) {
				if (duration == 0 || (lPlayed + 5 < duration)) {
					this.whenReadyToPlay = true;
					this.debug('记忆播放位置:', lPlayed);
					this.system.setUrl(this.video_data['url']);
					this.system.play();
				}
			}
		},
		switchClarity: function(no) {
			var me = this;
			var data = me.video_data;
			var system = me.system;
			if (data['resolutionNameNo'] == no || !me.store.isInDefinition(no)) {
				me.debug('当前不支持该清晰度切换', no);
			} else {
				me.debug('当前清晰度', no);
				var video = me.video_data = me.store.getVideoInfo(constants.DEFINITION_NO_NAME[no | 0]);
				var played = system.status.get_played();
				me.whenReadyToPlay = true;
				system.status.clear();
				system.status.setRememberKey(video.url);
				system.setUrl(video['url']);
				system.play(played);
				me.switchPlayTime = played;
				this.__filter_play();
			}
		},
		sdk_tecGet: function(attr, st) {
			...
			case ("allClaritys"):
				var key, all = this.store.getUniqueNoDefinition(),
					ret = [];
				for (key in all) {
					ret.push(all[key].resolutionNameNo);
				}
				return ret;
			case ('playState'):
		...
		sdk_operate: function(method, a, b, c) {
			...
			this.debug('operate', method);
			switch (method) {
			case ("resize"):
				var $main = $('#' + this.store.getIDMain());
				$main.width(a).height(b);
				$main.parent().width(a).height(b);
				this.control.setResize();
				break;
			case ("pause"):
				this.system.pause();
				break;
...
qcVideo('MediaPlayer_tpl', function() {
	return {
		'main': function(data) {
			...
			_p('<style>\r\n\
       #');
			_p(this.__escapeHtml(data.mainId));
			_p('{\r\n\
    ...
    <div id="');
		...
		'patch_image': function(data) {
			...
			_p('<a style="');
			_p(this.__escapeHtml(data.astyle));
			_p('" href="');
			_p(this.__escapeHtml(data.link));
			_p('" ');
			_p(this.__escapeHtml((data.link.indexOf('javascript') > -1 ? '' : ' target="_blank"')));
			_p('><img style="');
			_p(this.__escapeHtml(data.istyle));
			_p('" src="');
			_p(this.__escapeHtml(data.url));
			_p('"/></a>');
			return __p.join("");
		},
		'logo': function(data) {
			...
			_p('control-module" component="logo"></div>');
			return __p.join("");
		},
		'setting': function(data) {
			...
			_p('<div class="');
			_p(this.__escapeHtml(data.nick));
			_p('setting" component="setting" style="display:none;">\r\n\
           <div sub-component="close" class="');
			_p(this.__escapeHtml(data.nick));
			_p('setting-close">>></div>\r\n\
       </div>');
			return __p.join("");
		},
       ...
       <div class="setting-definition">');
			_p(this.__escapeHtml(data.name));
			_p('</div>\r\n\
       ...
       <div>');
			...
			for (var key in value) {
				var one = value[key];
				_p('            <div sub-component="definition" class="');
				_p(this.__escapeHtml(one.selected ? 'setting-definition-value-curr' : 'setting-definition-value'));
				_p('" data-def="');
				_p(this.__escapeHtml(one['sdk_no']));
				_p('">');
				_p(this.__escapeHtml(one['sdk_name']));
				_p('</div>');
			}
			_p('    </div>');
			return __p.join("");
		...
		'center': function(data) {
			...
			var i = data.nick;
			var dot = i + 'spinner-dot ' + i + 'spinner-dot';
			_p('    <div component="center_container">\r\n\
           <div class="');
			_p(this.__escapeHtml(i));
			_p('error" sub-component="error">\r\n\
   \r\n\
           </div>\r\n\
           <div class="');
			_p(this.__escapeHtml(i));
			_p('spinner" sub-component="loading" style="min-width:22px;min-height:22px;max-width:100px;max-height:100px;">\r\n\
               <svg height="100%" version="1.1" viewBox="0 0 22 22" width="100%">\r\n\
                   <svg x="7" y="1"><circle class="');
			_p(this.__escapeHtml(dot));
			_p('-1" cx="4" cy="4" r="2"></circle></svg>\r\n\
                   <svg x="11" y="3"><circle class=');
			_p(this.__escapeHtml(dot));
			_p('-2" cx="4" cy="4" r="2"></circle></svg>\r\n\
                   <svg x="13" y="7"><circle class="');
			_p(this.__escapeHtml(dot));
			_p('-3" cx="4" cy="4" r="2"></circle></svg>\r\n\
                   <svg x="11" y="11"><circle class="');
			_p(this.__escapeHtml(dot));
			_p('-4" cx="4" cy="4" r="2"></circle></svg>\r\n\
                   <svg x="7" y="13"><circle class="');
			_p(this.__escapeHtml(dot));
			_p('-5" cx="4" cy="4" r="2"></circle></svg>\r\n\
                   <svg x="3" y="11"><circle class="');
			_p(this.__escapeHtml(dot));
			_p('-6" cx="4" cy="4" r="2"></circle></svg>\r\n\
                   <svg x="1" y="7"><circle class="');
			_p(this.__escapeHtml(dot));
			_p('-7" cx="4" cy="4" r="2"></circle></svg>\r\n\
                   <svg x="3" y="3"><circle class="');
			_p(this.__escapeHtml(dot));
			_p('-8" cx="4" cy="4" r="2"></circle></svg>\r\n\
               </svg>\r\n\
           </div>\r\n\
           <div class="');
			_p(this.__escapeHtml(i));
			_p('play-controller" sub-component="play" style="width: 98px;margin: -49px 0 0 -49px;">\r\n\
		...
		'bottom': function(data) {
			...
			var nick = data.nick;
			_p('    <!--底部控制栏-->\r\n\
       <div data-resize-module="bottom" class="');
           ...
           <div style="width:99%;height:36px;margin:0px auto;position:relative;  background: #000;">\r\n\
               ...
               <div class="');
			_p(this.__escapeHtml(nick));
			_p('progress-bar-container">\r\n\
                   <div class="');
			_p(this.__escapeHtml(nick));
			_p('progress-bar" aria-label="播放滑块" style="touch-action: none;');
			_p(this.__escapeHtml(data.version.IS_MOBILE ? 'display:none' : ''));
			_p('">\r\n\
                       <div class="');
			_p(this.__escapeHtml(nick));
			_p('progress-list" sub-component="progress_bg">\r\n\
                           <div class="');
			_p(this.__escapeHtml(nick));
			_p('play-progress ');
			_p(this.__escapeHtml(nick));
			_p('in-bg-color" style="/*transform: scaleX(0.001);*/width:1%;"\r\n\
                                sub-component="progress_play"></div>\r\n\
                           <div class="');
			_p(this.__escapeHtml(nick));
			_p('load-progress" style="/*transform: scaleX(0.001);*/width:1%;" sub-component="progress_load"></div>\r\n\
                       </div>\r\n\
                       <div class="');
			_p(this.__escapeHtml(nick));
			_p('scrubber-button ');
   ...
   \r\n\
               <div class="');
			_p(this.__escapeHtml(nick));
			_p('bottom-controls" style="');
			_p(this.__escapeHtml(data.version.IS_MOBILE ? 'border-top: solid 1px rgba(255, 255, 255, .2)' : ''));
			_p('">\r\n\
                   <!--播放-->\r\n\
                   <button class="');
			_p(this.__escapeHtml(nick));
			_p('button" style="width:36px;height:36px;float:left;" title="');
			_p(this.__escapeHtml(data.WORD.PLAY));
			_p('" sub-component="play_btn" now="play">\r\n\
			...
			if (data['is_max_screen']) {
				_p('\r\n\
                       <!--静音-->\r\n\
                       <div class="');
				_p(this.__escapeHtml(nick));
				_p('volume-control" style="float:left;">\r\n\
                           <button class="');
				_p(this.__escapeHtml(nick));
				_p('button" style="width:36px;height:36px;" title="');
				_p(this.__escapeHtml(data.WORD.MUTE));
				_p('" sub-component="volume_mute" now="volume">\r\n\
                               <!--SVG底部 音量 按钮-->\r\n\
   ...
   \r\n\
                           <div class="');
				_p(this.__escapeHtml(nick));
				_p('volume-panel" title="');
				_p(this.__escapeHtml(data.WORD.TIP_OPR_VOLUME));
				_p('" style="">\r\n\
                               <div class="');
				_p(this.__escapeHtml(nick));
				_p('volume-slider" style="touch-action: none;" sub-component="volume_bg" >\r\n\
                                   <div class="');
				_p(this.__escapeHtml(nick));
				_p('volume-slider-track-after"></div>\r\n\
                                   <div class="');
				_p(this.__escapeHtml(nick));
				_p('volume-slider-track ');
				_p(this.__escapeHtml(nick));
				_p('in-bg-color" style="width: 24px;" sub-component="volume_track"></div>\r\n\
                                   <div class="');
				_p(this.__escapeHtml(nick));
				_p('volume-slider-handle" style="left: 24px;" sub-component="volume_slider" draggable="true"></div>\r\n\
                               </div>\r\n\
                           </div>\r\n\
                       </div>');
			}
			_p('\r\n\
                   ...
                   <div class="');
			_p(this.__escapeHtml(nick));
			_p('time-progress" style="float:left;">\r\n\
                       <span class="');
			_p(this.__escapeHtml(nick));
			_p('in-color" sub-component="time_played">00:00:00</span>\r\n\
			...
			_p('button ');
			_p(this.__escapeHtml(nick));
			_p('control-left" style="right: 5px;" title="');
			_p(this.__escapeHtml(data.WORD.FULL_SCREEN));
			_p('" sub-component="fullscreen_btn" now="fullscreen">\r\n\
                       <!--SVG底部 全屏 按钮-->\r\n\
   ...
   \r\n\
			...
			_p('button ');
			_p(this.__escapeHtml(nick));
			_p('control-left" style="right: 41px;" title="');
			_p(this.__escapeHtml(data.WORD.SETTING));
			_p('" sub-component="setting">\r\n\
		...
		'controller': function(data) {
			...
			_p(this.style(data));
			_p('    <div h5-controller id="');
			_p(this.__escapeHtml(data.controller.id));
			_p('" style="position: absolute;top: 0px;left: 0px;width: 100%;height: 100%;z-index:3;">\r\n\
           <!--<div style="position:abolute;width:100%;height:100%;z-index:2" data-mask="video"></div>  -->');
			_p(this.center(data));
			_p('        ');
			_p(this.bottom(data));
			_p('        ');
		...
		'video': function(data) {
			...
			_p('<div style="position: relative;width: 100%;height: 100%;  overflow: hidden;-webkit-box-sizing: border-box;box-sizing: border-box;">\r\n\
           <video\r\n\
		...
		'style': function(data) {
			...
			_p('<style>');
			_p(this.style_center(data));
			_p('        ');
			_p(this.style_bottom(data));
			_p('        ');
			_p(this.style_setting(data));
			_p('    </style>');
			return __p.join("");
		},
      ...
      .');
			_p(this.__escapeHtml(data.nick));
			_p('setting .setting-definition-value{cursor: pointer;}\r\n\
      .');
			_p(this.__escapeHtml(data.nick));
			_p('setting .setting-definition-value-curr{cursor: default;border: 2px solid rgb(0,160,233);border-radius: 2px;color: rgb(0,160,233);}');
			return __p.join("");
		},
       ...
       .');
			_p(this.__escapeHtml(nick));
			_p('control-module{position: absolute;top: 0px;left: 0px;}\r\n\
   \r\n\
       .');
			_p(this.__escapeHtml(nick));
			_p('play-controller{\r\n\
   ...
   \r\n\
       ...
       .');
			...
			for (var i = 0; i < keyFrames.length; i++) {
				_p('        ');
				_p(this.__escapeHtml(keyFrames[i]));
				_p(' ');
				_p(this.__escapeHtml(nick));
				_p('spinner-dot-fade {\r\n\
               0% {\r\n\
   ...
   \r\n\
      .');
          ...
          -moz-animation: ');
			_p(this.__escapeHtml(nick));
			_p('spinner-dot-fade .8s ease infinite;\r\n\
          -webkit-animation: ');
			_p(this.__escapeHtml(nick));
			_p('spinner-dot-fade .8s ease infinite;\r\n\
          animation: ');
          ...
          transform-origin: 4px 4px\r\n\
			...
			for (; i < 7; i++) {
				...
				_p(this.__escapeHtml(nick));
				_p('spinner-dot-');
				_p(this.__escapeHtml(i + 1));
				_p(' { -moz-animation-delay: .');
				_p(this.__escapeHtml(i + 1));
				_p('s; -webkit-animation-delay: .');
				_p(this.__escapeHtml(i + 1));
				_p('s; animation-delay: .');
				_p(this.__escapeHtml(i + 1));
				_p('s }');
			}
			_p('   /**转菊花结束**/');
		...
		'style_bottom': function(data) {
			...
			var i = '#' + data.controller.id;
			var nick = data.nick;
			_p('        ');
			_p(this.__escapeHtml(i));
			_p(' button, ');
			_p(this.__escapeHtml(i));
			_p(' svg { margin: 0px; padding: 0px; }\r\n\
   \r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('control-bottom {\r\n\
   ...
   \r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('control-left{width:36px;height:36px;position: absolute;z-index: 3;bottom: 0px; }\r\n\
   \r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('control-bottom, .');
   ...
   \r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('progress-bar-container:hover{height:15px; }\r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('progress-bar-container:hover .');
			_p(this.__escapeHtml(nick));
			_p('scrubber-button{ top: 1px; height: 15px;width: 15px;}\r\n\
   \r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('progress-bar {\r\n\
   ...
   \r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('play-progress { z-index: 34; }\r\n\
   \r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('load-progress { z-index: 33; background: rgba(255, 255, 255, .4); }\r\n\
   \r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('scrubber-button {\r\n\
   ...
   \r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('bottom-controls { height: 36px; line-height: 36px; text-align: left; direction: ltr;  position: relative;overflow: hidden; }\r\n\
   \r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('button {\r\n\
   ...
   \r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('in-bg-color { background: #1275cf; }\r\n\
   \r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('in-color { color: #1275cf; }\r\n\
   \r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('volume-control { display: inline-block; }\r\n\
   \r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('volume-panel{\r\n\
   ...
   \r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('volume-slider { height: 100%; position: relative; }\r\n\
   \r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('volume-slider-track{\r\n\
   ...
   \r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('time-progress { color: #ddd; display: inline-block; vertical-align: top; }\r\n\
   \r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('control-bottom { opacity: 0.5; }\r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('scrubber-button { opacity: 0; }\r\n\
   \r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('control-bottom-flow:hover, .');
			_p(this.__escapeHtml(nick));
			_p('control-bottom-stick:hover,\r\n\
           .');
			_p(this.__escapeHtml(nick));
			_p('control-bottom-flow:hover .');
			_p(this.__escapeHtml(nick));
...
qcVideo('PlayerStatus', function(Base, constants, lStore) {
	...
	return Base.extend({
		...
		start_duration: function(second) {
			if (second !== undefined) {
				this._start_duration = second;
			} else {
				return this._start_duration;
			}
		},
		check: function() {
			var me = this;
			if (me.__isMaybeBlockStatus()) {
				...
				if (dif > blockTime && me.__getStatusValue() === me.status_value && !me.inCache(me.played)) {
					me.debug('overtime', dif);
		...
		set_played: function(num) {
			this.played = num - 0;
			this.fire(EVENT.OS_TIME_UPDATE);
			if (this.remember) {
				lStore.set(this.rememberKey + '_played', this.get_played());
				this.get_local_played();
			}
		},
		set_volume: function(num) {
			this._volume = num;
			if (this.remember) {
				lStore.set('volume', num);
				this.get_local_volume();
			}
		},
		get_volume: function() {
			return this._volume || (lStore.get('volume') | 0) || 0.5;
		},
		...
		inCache: function(second) {
			...
			var start = this.start_duration();
			this.debug(loaded, start, second);
			if (this.loaded > 0 && second <= loaded + 5 && second >= start) {
				this.debug('in mem');
				return true;
			}
			this.debug('out mem');
	...
	return Base.extend({
		...
		_getPoster: function(pos) {
			var image_url = this.store.image_url,
				...
				maxPosition = 0,
				tempSize = 0;
			if (patch && patch[pos] && patch[pos]['url']) {
				return patch[pos];
			}
			if (pos == constants.PATCH_LOC.START && image_url && image_url.indexOf('p.qpic.cn') == -1) {
				return {
					url: image_url,
					type: pos,
					redirect: ''
			...
			if (image_url && imgs && imgs.length > 0 && pos == constants.PATCH_LOC.START) {
				var user_choose_url = image_url.substring(0, image_url.lastIndexOf('/'));
				for (var i = 0, j = imgs.length; i < j; i++) {
					tempSize = (imgs[i]['vheight'] | 0) * (imgs[i]['vwidth'] | 0);
					if (tempSize > maxSize && imgs[i].url.indexOf(user_choose_url) != -1) {
						maxSize = tempSize;
						maxPosition = i;
					}
				}
		...
		happenToSdk: function(arg) {
			...
			if (me.__happen_last_time_status != arg) {
				...
				TDBankReporter.pushEvent(playStatus, me.store);
				me.__happen_last_time_status = arg;
				window[this.store.sdk_method](arg);
			}
		},
		isCustomization: function(n) {
			return this.store.customization[n] == true;
		},
		...
		getMainOffset: function() {
			...
			offset.width = $main.width();
			offset.height = $main.height();
			return offset;
		},
		getStartPatch: function() {
			return this._getPoster(constants.PATCH_LOC.START | 0);
		},
		getPausePatch: function() {
			return this._getPoster(constants.PATCH_LOC.PAUSE | 0);
		},
		...
		getUniqueNoDefinition: function() {
			...
			var unique = {};
			for (var key in all) {
				if (!unique[all[key]['resolutionNameNo']]) {
					unique[all[key]['resolutionNameNo']] = true;
					ret[key] = all[key];
				}
			}
			return ret;
		...
		getVideoInfo: function(res, def) {
			var me = this,
				...
				getRstFromDefPriority = function(res) {
					...
					if (arr && arr.length > 0) {
						return videos[arr[0]] || videos[arr[1]] || null;
					}
				};
			var all = this.getAllDefinition();
			me.debug(':get video:resource==' + res + ';definition==' + def);
			if (def !== undefined) {
				rst = videos[def | 0];
				if (!all[def]) {
					rst = null;
			...
			if (!rst && res !== undefined) {
				rst = getRstFromDefPriority(res | 0);
			}
			if (!rst) {
			...
			if (rst) {
				...
				me.debug(':getVideoInfo result:清晰度==' + map['resolution'] + ';名称==' + map['resolutionName'] + ';sdk清晰度' + map['resolutionNameNo']);
				return util.merge({
...
qcVideo('PlayerSystem', function($, Base, interval, constants, util, FullScreenApi, version, JSON) {
	...
	var EventAry = 'loadstart,suspend,abort,error,emptied,stalled,loadedmetadata,loadeddata,canplay,canplaythrough,playing,waiting,seeking,seeked,ended,durationchange,timeupdate,progress,play,pause,ratechange,volumechange'.split(',');
	var orientationMap = {};
	var orientationChange = function() {
			var key;
			switch (window.orientation) {
			case 180:
				for (key in orientationMap) {
					if (orientationMap[key] && orientationMap[key]['happenResize']) {
						orientationMap[key]['happenResize'](true);
					}
				}
				break;
			case 90:
				for (key in orientationMap) {
					if (orientationMap[key] && orientationMap[key]['happenResize']) {
						orientationMap[key]['happenResize'](false);
					}
				}
				break;
			}
	...
	return Base.extend({
		...
		destroy: function() {
			delete orientationMap[this.__system_id];
		...
		__getBuffer: function() {
			var end = 0;
			try {
				for (var i = 0, j = this.video.buffered.length; i < j; i++) {
					end = this.video.buffered.end(i) | 0;
				}
			} catch (e) {}
			return end;
		},
		...
		_bind: function() {
			var me = this;
			var metadataDone = function() {
					me.metadatadone = true;
					if (me._seek_time > 1 && me.video.currentTime < 1) {
						me.video.currentTime = me._seek_time;
					}
					if (me._volume) {
						me.video.volume = this._volume;
					}
					me._seek_time = 0;
			...
			var getHandler = function(event) {
					return function(e) {
						me.debug(event, me.has_call_play);
						if (!me.has_call_play) {
							return;
						}
						switch (event) {
						...
						case ('progress'):
							...
							me.status.set_loaded(me.__getBuffer());
							break;
						case ('timeupdate'):
							me.status.set_played(me.video.currentTime | 0);
							break;
						case ('ended'):
							me.fire(EVENT.OS_PLAYER_END);
							break;
		...
		setUrl: function(src) {
			var me = this;
			var $renderTo = me.$renderTo;
			var type = me.fileType = util.fileType(src);
			me.__system_id = getId();
			orientationMap[me.__system_id] = me;
			var tpl = {
				vid: me.__system_id,
			...
			if (me.$video) {
				...
				me.video.src = '';
				if (me._hls) {
					me._hls.detachMedia(me.video);
				}
				me.callMethod('pause');
				me.$video.remove();
			}
			var tplObj = qcVideo.get('MediaPlayer_tpl');
			$renderTo.prepend(tplObj['video'](tpl));
			me.video = (me.$video = $('#' + tpl.vid)).get(0);
			me._bind();
			me._toPlayOrPauseView(false);
			if (version.REQUIRE_HLS_JS && type == constants.HLS) {
				var Hls = qcVideo.get('Hls');
				me._hls = new Hls();
				me._hls.loadSource(src);
				me._hls.attachMedia(me.video);
			}
		},
		...
		setFullScreen: function(isFull, targetID) {
			...
			var $main = $('#' + targetID),
				$win = $(window);
			var target = isFull ? $main.get(0) : document;
			if (isFull) {
				var offset = $main.offset();
				target[FullScreenApi['requestFullscreen']]();
				setTimeout(function() {
					if (me.isFullScreen($main)) {
						me.__offset = offset;
						$main.width($win.width());
						$main.height($win.height());
						me.fire(EVENT.OS_RESIZE, {});
					}
				}, 250);
			} else {
				target[FullScreenApi['exitFullscreen']]();
				if (me.__offset) {
					$main.width(me.__offset.width);
					$main.height(me.__offset.height);
				}
			}
		},
		...
		volume: function(volume) {
			if (volume !== undefined) {
				if (this.video) {
					...
					this.fire(EVENT.OS_VOLUME_CHANGE, {
						'volume': volume
					});
				}
			} else {
...
qcVideo('UiControl', function($, Base, util, constants, version) {
	var id = 'qvideo_control_' + (+new Date()) + '_';
	var uid = 1;
	var bottomLeft = 10;
	var MODE;
	var UiControl = Base.extend({
		className: 'UiControl',
		destroy: function() {
			var me = this;
			delete me.store;
		...
		__ableHideUi: function() {
			return this.__on_ui && this.status.getSDKStatus() == 'playing' && !this.__hover_controller && (+new Date() - this.__show_ui_time > 2000);
		},
		__delayHideUi: function() {
			var me = this;
			if (me.status.getSDKStatus() == 'playing' && !me.__hover_controller) {
				me.__show_ui_time = +new Date();
				if (me.__hide_ui_tid) {
					clearTimeout(me.__hide_ui_tid);
					me.__hide_ui_tid = null;
				}
				me.__hide_ui_tid = setTimeout(function() {
					if (me.__ableHideUi()) {
						me.__on_ui = false;
						me.children['Bottom_container'].$el.hide();
						me.log('hide');
					}
				}, 2080);
			}
		...
		enterPlayerUi: function() {
			if (this.__ableShowUi()) {
				this.__on_ui = true;
				this.children['Bottom_container'].$el.show();
				this.log('show');
			}
			this.__delayHideUi();
		},
		...
		constructor: function(store, status, $renderTo) {
			...
			$renderTo.find('[component]').each(function() {
				...
				me.children[component] = new UICom(store, status, $me);
				me.on(me.children[component], constants.FIRE, function(obj) {
				...
				me.children[component].live('[sub-component]', function(dom, e) {
					if (this.enable()) {
						...
						if (util.type(this[method]) == 'function') {
							this[method]($dom, e);
						}
					}
				});
			});
			me.$el = $renderTo.find('div[h5-controller]');
			if (!version.IS_MOBILE) {
				$renderTo.off('mouseenter').off('mouseleave').off('mousemove').on('mouseenter', function(e) {
					me.enterPlayerUi();
				}).on('mouseleave', function() {
					me.leavePlayerUi();
			...
			me.children['Bottom_container'].$el.on('mouseenter', function(e) {
				me.__hover_controller = true;
			}).on('mouseleave', function(e) {
				me.__hover_controller = false;
			});
			me.setViewMode(height);
			this.setWait();
			this.setTime({
				duration: me.store.getDuration()
			});
			this.setResize();
			if (me.store.isCustomization('hide_h5_setting')) {
				me.$el.find('[sub-component="setting"]').hide();
			}
		},
		setViewMode: function(height) {
			var me = this;
			var sourceH = 36;
			var $settingEl = this.children['Setting'].$el;
			if (version.IS_MOBILE && height > 0) {
				...
				var zoom = 1;
				if (rate != askRate) {
					zoom = height * askRate / sourceH;
				}
				this.children['Bottom_container'].$el.css('zoom', zoom);
				setTimeout(function() {
					var h = $settingEl.height();
					var ch = me.$el.height() || 1;
					var rate = h / ch;
					var size = '1rem';
					if (rate < 0.3) {
						size = '2rem';
					...
					me.log(h, height, ch, rate, size);
					$settingEl.css('font-size', size).height('100%').hide();
				}, 300);
			} else {
				$settingEl.height('100%')
			}
		},
		show: function() {
			this.$el.show();
