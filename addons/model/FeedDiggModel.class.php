	...
	public function addDigg($feed_id, $mid)
	{
		...
		if($res){
			...
			if($mid != $feed['uid']){
		    	...
		    	if ($feed['is_repost'] == 1) {
		        	$config['content'] = $feed['feed_content'];
		    	}else {
		    		$source = model('Source')->getSourceInfo($feed['app_row_table'], $feed['app_row_id']);
		        	switch ($feed['app']) {
		                case 'blog':
		               		$config['content'] = '您的知识<a href="'.$source['source_url'].'" target="_blank">'.$source['title'].'</a>';
		                    break;
		                case 'photo':
		                    $config['content'] = '您的相册<a href="'.$source['album_url'].'" target="_blank">['.$source['title'].']</a>';
		                    break;
		                case 'event':
		                    $config['content'] = '您的活动<a href="'.$source['source_url'].'" target="_blank">'.$source['title'].'</a>';
		                    break;
		                case 'vote':
		                    $config['content'] = '您的投票<a href="'.$source['source_url'].'" target="_blank">'.$source['title'].'</a>';
		                    break;
		                case 'weiba':
		                    $config['content'] = '您的帖子<a href="'.$source['source_url'].'" target="_blank">'.$source['title'].'</a>';
		                    break;
		                case 'public':
		                default :
		                    $config['content'] = $feed['feed_content'];
				...
				model ( 'Notify' )->sendNotify ( $feed['uid'], 'digg', $config, $mid );
			}
